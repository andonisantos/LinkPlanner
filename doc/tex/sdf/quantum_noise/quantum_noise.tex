...
++ (VER O PAPER do FourYearReview\_pub92.pdf) ++\\
++ (VER PAPER DO Quantum noise and excess noise in optical homodyne... do SHAPIRO 1985) ++ \\
...
%\\
%(Hans, p. 13??)\\
Classicaly, the electric field of an ideal single mode laser, modulated in amplitude and phase by a signal $A(t)$, incident in a photodiode is represented as
%%
%%
\begin{equation}
...
Complex notation\\
\\
To simplify calculations, the electric field can be expressed by using complex notation. The physically measurable quantities are obtained by taking the real part of the complex wave
...
\\
Using this notation, the beam power ($P(t)$) is obtained by multipliyng the electric field's conjugate by itself
\footnote{REF?}
%%
\begin{align}
	P(t) &= E^{*}(t) E(t)\nonumber\\
...
Given the previous definition of electric field (ref?) and power (ref?), we will obtain the optical power as
%%
\begin{equation}
	E(t) = A(t) e^{-i \omega_0}
\end{equation}
...
\begin{figure}[H]
	\centering
	\includegraphics{./sdf/quantum_noise/figures/detection-balanced-homodyne.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=15cm]{./sdf/quantum_noise/figures/detection-IQ-balanced-homodyne.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics{./sdf/quantum_noise/figures/detection-heterodyne.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics{./sdf/quantum_noise/figures/detection-balanced-heterodyne.pdf}
...
%%
\vspace{4em}
++ A DESCRICAO QUANTICA COMECA AQUI ++\\
%%
...
%%
%%
A transimpedance amplifier (TIA) "converts the photodiode's current output signal to a usable voltage level"
\footnote{https://www.digikey.com/en/articles/techzone/2017/jul/design-transimpedance-amplifiers-for-precision-opto-sensing}\\
++ SEM REFERENCIA ++\\
The TIA has a high impedance??? which is good to measurent instruments. \footnote{REF?}
++ WIKI ++\\
Current-to-voltage converters are used with sensors that have a current response that is more linear than the voltage response. This is the case with photodiodes,
%%
\noindent
{\bf Gain response}\\
\\
