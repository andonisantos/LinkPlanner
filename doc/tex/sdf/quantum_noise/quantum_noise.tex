...
Complex notation
\\
%\footnote{Mark Fox, p.130}
"The use o complex notation simplifies the mathematics (...). Physically measurable quantities are obtained by taking the real part of the complex wave"
\footnote{Mark Fox, p.11}
\\
Using real notation,the electric field is express as $E(t) = |A(t)|\cos(\omega t + \theta )$ A more compact way of expressing the electric field is the complex notation
%%
\begin{equation}
	E(t) = A(t) e^{-i \omega_0}
\end{equation}
...
\\
%To recover the real notation, we simply use $E(t) = \textrm{Re} \left[ A(t) e^{-i \omega0} \right] $.\\
-- REFERENCIAS --\\
The beam power (P) can be recoverd using the product between the field conjugate and the field itself
%
\begin{align}
	P(t) &= E^{*}(t) E(t)\nonumber\\
         &= |A(t)|^2
...
(Hans, p. 181)\\
The photocurrent $i(t)$ is obtained by 
...
in which $\eta$ is the photodiode's responsivity, q is the unit charge and $P(t)/\hbar \omega_0$ is the number of photons.\\
...
In most cases? the detection will be proportional to the intensity of the light beam.\\

Therefore, the intensity of 
A beam of light with a well defined frequency can be defined as:\\
\\
\\
\\
\\
-- ESTA EQUACAO TA MAL --\\
\begin{equation}
E = E_0\left[ \exp(i\omega t + i\theta) + \exp(-i\omega t - i\theta) \right]
\end{equation}
\\
\\
Single Field Detection\\
