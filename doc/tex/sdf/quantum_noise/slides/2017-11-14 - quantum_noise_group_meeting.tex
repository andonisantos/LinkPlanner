...
The studied communication system is based on the transmission of coherent states. Coherent states can be defined in the number state basis $\{\ket{n}\}$ as
$$
...
The measurement of quadratures is made by the homodyne technique. The quantum description of the detection is described by the following diagram
...
\noindent
The used technique is the double homodyne technique which starts by dividing S in two beams of equal power. One the beams is measures in-phase with the local oscillator, and the other is measured with a phase difference of $\pi/2$ relative to the local oscillator.\\
Given the input signal state $\ket{\alpha}$ ($\alpha = |\alpha| e^{i\theta_\alpha}$) and the local oscillator state  $\ket{\beta}$ ($\beta = |\beta| e^{i\theta_\beta}$), the expected value for the two quadratures operators $\hat{m_X}$ and $\hat{m_Y}$ is\\
%
$$
	\braket{\hat{m}_X} = \left|\frac{\alpha}{\sqrt{2}}\right| \cos({\theta_\alpha - \theta_\beta}),
	\qquad
	\textrm{Var}(\hat{m}_X) \approx \frac{1}{4}
$$
$$
	\braket{\hat{m}_Y} =  \left|\frac{\alpha}{\sqrt{2}}\right| \sin({\theta_\alpha - \theta_\beta}),
	\qquad
	\textrm{Var}(\hat{m}_Y) \approx \frac{1}{4}
$$
...
A signal is produced by modulating and attenuating a laser beam. This signal and a vacuum signal are used as inputs of the Coherent Receiver, outputing a voltage proportional to the homodyne detection.
...
These plots of the variance of $\hat{m}$ plots, for two quadratures, show a constant value for low power of LO, and a growth proportional to the square of the power of LO for higher power. This is not in conformity with the theory and simulation.
