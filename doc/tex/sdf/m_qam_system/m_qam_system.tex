...
This shows that the SNR and, therefore, the error probability are dependent on the energy of each symbol and the noise spectral density. However, even though the relation does not directly depend on the pulse shape, the energy of each symbol still depends on the pulse shape. It can also be expected that the energy is related to the symbol period. Considering two a similar shaped pulses, with similar amplitude but duration, the one with longer duration should have more energy.
