...
Considering a pulse with composition similar to the signal described by Equation~\ref{eq:sigAmpNoise}, let $ a(t) = A_p~p(t) $, where $A_p$ is the peak amplitude of the signal and $p(t)$ is the shape of the pulse. Also, let $H(f)$ be the frequency domain response of the filter at the receiver. For matched filtering, the receiver filter is equal to the transmitter, but reversed in time and shifted by a time delay~\cite{carlson1986communication}.

The energy contained in the pulse depends on the pulse amplitude and on its shape, and it is given by:
...
\begin{eqnarray}
A_o = \mathfrak{F}^{-1}\left[H(f) A(f)\right]\big|_{t=t_0+t_d} = A \int_{-\infty}^{+\infty} H(f) P(f) e^{+j \omega t_d}df
\end{eqnarray}

Similarly, considering the noise at the input to be AWGN  with spectral density $G_n = \frac{n_0}{2}$, the noise power at the receiver filter output is given by:
